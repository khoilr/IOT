...
def data_drop_top_k(model, data_path, dest_file=None, deg_thresh=75, limit=0):
    ...
    image_names, image_classes, _, _, dataset_pickle_file_src = pre_process_folder(
        data_path
    )  # Reload from pickle file
    sorted_idx = np.argsort(image_classes)
    image_names, image_classes = (
        np.array(image_names)[sorted_idx],
        np.array(image_classes)[sorted_idx],
    )
    ...
    if isinstance(model, str):
        ...
        mm = tf.keras.models.load_model(
            model, custom_objects={"NormDense": NormDense}, compile=False
        )
    else:
        mm = model
    ...
    print(
        ">>>> [Before] emb_num = %d, class_num = %d, top_k = %d, images = %d"
        % (emb_num, class_num, top_k, len(image_classes))
    )
    ...
    for ii in tqdm(range(total_idxes)):
        ...
        max_sub_center_count = [
            (max_sub_center_idxes == idx).sum() for idx in range(top_k)
        ]
    ...
    """ Do shuffle again """
    new_image_classes, new_image_names = np.array(new_image_classes), np.array(
        new_image_names
    )
    shuffle_idxes = np.random.permutation(len(new_image_names))
    new_image_classes, new_image_names = (
        new_image_classes[shuffle_idxes].tolist(),
        new_image_names[shuffle_idxes].tolist(),
    )
    ...
    np.savez_compressed(
        dest_file, image_names=new_image_names, image_classes=new_image_classes
    )
    # with open(dest_file, "wb") as ff:
    #     pickle.dump({"image_names": new_image_names, "image_classes": new_image_classes}, ff)

    print(
        ">>>> [After] Total classes: %d, total images: %d"
        % (np.max(new_image_classes) + 1, len(new_image_names))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Saved model file path, NOT basic_model",
    )
    parser.add_argument(
        "-D", "--data_path", type=str, required=True, help="Original dataset path"
    )
    parser.add_argument(
        "-d",
        "--dest_file",
        type=str,
        default=None,
        help="Dest file path to save the processed dataset npz",
    )
    parser.add_argument(
        "-t",
        "--deg_thresh",
        type=int,
        default=75,
        help="Thresh value in degree, [0, 180]",
    )
    parser.add_argument(
        "-L",
        "--limit",
        type=int,
        default=0,
        help="Test parameter, limit converting only the first [NUM] ones",
    )
    ...
    print(
        ">>>> Output:",
        data_drop_top_k(
            args.model_file, args.data_path, args.dest_file, args.deg_thresh, args.limit
        ),
    )
