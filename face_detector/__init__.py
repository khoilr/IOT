...
DEFAULT_ANCHORS = np.array(
    [
        ...
        [
            [4.5625, 6.781199932098389],
            [7.218800067901611, 9.375],
...
class BaseDetector:
    ...
    def detect_in_image(
        ...
        if isinstance(image, str):
            ...
            image_format = "RGB"
        bbs, pps, ccs = self.__call__(image, max_output_size, iou_threshold, score_threshold, image_format)  # type: ignore
    ...
    def detect_in_folder(self, data_path, max_output_size=15, iou_threshold=0.45, score_threshold=0.25):
        ...
        for imm in tqdm(imms, "Detecting"):
            _, _, _, nimages = self.detect_in_image(
                imm, max_output_size, iou_threshold, score_threshold, image_format="RGB"
            )
            if nimages.shape[0] != 0:
                file_name = os.path.basename(imm)  # type: ignore
                if use_class:
                    class_name = os.path.basename(os.path.dirname(imm))  # type: ignore
...
class YoloV5FaceDetector(BaseDetector):
    ...
    def post_process(self, outputs, image_height, image_width):
        ...
        for output, stride, anchor, anchor_grid in zip(outputs, self.strides, self.anchors, self.anchor_grids):
            ...
            output = tf.reshape(
                output,
                [-1, output.shape[1] * output.shape[2], self.num_anchors, anchor_width],
            )
    ...
    def yolo_nms(self, inputs, max_output_size=15, iou_threshold=0.35, score_threshold=0.25):
        ...
        bbs, pps, ccs = (
            tf.gather(bbt, rr, axis=0),
            tf.gather(ppt, rr, axis=0),
            tf.gather(cct, rr, axis=0),
        )
    ...
    def __call__(
        ...
        outputs = self.model(imm_RGB)  # type: ignore
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_scrfd",
        action="store_true",
        help="Use SCRFD instead of YoloV5FaceDetector",
    )
