...
class MarginSoftmax(tf.keras.losses.Loss):
    def __init__(
        ...
        **kwargs
        ...
        self.power, self.scale, self.from_logits, self.label_smoothing = (
            power,
            scale,
            from_logits,
            label_smoothing,
    ...
    def call(self, y_true, y_pred):
        ...
        margin_soft = (
            tf.where(
                ...
                self.logits_reduction_func(y_pred),
                y_pred,
            )
            * self.scale_all
        )
...
class ArcfaceLoss(tf.keras.losses.Loss):
    def __init__(
        ...
        **kwargs
        ...
        self.margin1, self.margin2, self.margin3, self.scale = (
            margin1,
            margin2,
            margin3,
            scale,
    ...
    def build(self, batch_size):
        self.batch_labels_back_up = tf.Variable(
            tf.zeros([batch_size], dtype="int64"), dtype="int64", trainable=False
        )
    ...
    def call(self, y_true, norm_logits):
        ...
        if self.margin1 == 1.0 and self.margin2 == 0.0 and self.margin3 == 0.0:
            ...
            theta = (
                tf.cos(tf.acos(y_pred_vals) * self.margin1 + self.margin2)
                - self.margin3
            )
            # Grad(theta) == 0
        ...
        theta_valid = tf.where(
            y_pred_vals > self.threshold, theta, self.theta_min - theta
        )
        ...
        arcface_logits = (
            tf.tensor_scatter_nd_update(norm_logits, pick_cond, theta_valid)
            * self.scale
        )
...
class ArcfaceLossSimple(tf.keras.losses.Loss):
    def __init__(
        self, margin=0.5, scale=64.0, from_logits=True, label_smoothing=0, **kwargs
    ):
        super(ArcfaceLossSimple, self).__init__(**kwargs)
        self.margin, self.scale, self.from_logits, self.label_smoothing = (
            margin,
            scale,
            from_logits,
            label_smoothing,
    ...
    def call(self, y_true, norm_logits):
        ...
        y_pred_vals = tf.gather_nd(norm_logits, pick_cond)
        theta = (
            y_pred_vals * self.margin_cos
            - tf.sqrt(1 - tf.pow(y_pred_vals, 2)) * self.margin_sin
        )
        theta_valid = tf.where(
            y_pred_vals > self.threshold, theta, self.theta_min - theta
        )
...
class CurricularFaceLoss(ArcfaceLossSimple):
    def __init__(
        ...
        **kwargs
        ...
        super(CurricularFaceLoss, self).__init__(
            margin, scale, from_logits, label_smoothing, **kwargs
        )
    ...
    def call(self, y_true, norm_logits):
        ...
        self.hard_scale.assign(
            tf.reduce_mean(y_pred_vals) * 0.01 + (1 - 0.01) * self.hard_scale
        )
        tf.print(", hard_scale:", self.hard_scale, end="")
        hard_norm_logits = tf.where(
            norm_logits > tf.expand_dims(theta_valid, 1),
            norm_logits * (self.hard_scale + norm_logits),
            norm_logits,
        )

        arcface_logits = (
            tf.tensor_scatter_nd_update(hard_norm_logits, pick_cond, theta_valid)
            * self.scale
        )
        return tf.keras.losses.categorical_crossentropy(
            y_true,
...
class AirFaceLoss(ArcfaceLossSimple):
    def __init__(
        self, margin=0.4, scale=64.0, from_logits=True, label_smoothing=0, **kwargs
    ):
        super(AirFaceLoss, self).__init__(
            margin, scale, from_logits, label_smoothing, **kwargs
        )
...
class CosFaceLoss(ArcfaceLossSimple):
    def __init__(
        self, margin=0.35, scale=64.0, from_logits=True, label_smoothing=0, **kwargs
    ):
    ...
    def call(self, y_true, norm_logits):
        ...
        pick_cond = tf.cast(y_true, dtype=tf.bool)
        logits = (
            tf.where(pick_cond, norm_logits - self.margin, norm_logits) * self.scale
        )
        return tf.keras.losses.categorical_crossentropy(
            y_true,
            logits,
...
class MagFaceLoss(ArcfaceLossSimple):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # l_a, u_a, lambda_g
        self.min_feature_norm, self.max_feature_norm, self.regularizer_loss_lambda = (
            min_feature_norm,
            max_feature_norm,
            regularizer_loss_lambda,
        )
        # l_margin, u_margin
        self.min_margin, self.max_margin = min_margin, max_margin
        self.use_cosface_margin, self.curricular_hard_scale = (
            use_cosface_margin,
            curricular_hard_scale,
        )
        ...
        if curricular_hard_scale >= 0:
            self.curricular_hard_scale = tf.Variable(
                curricular_hard_scale, dtype="float32", trainable=False
            )
    ...
    def call(self, y_true, norm_logits_with_norm):
        ...
        norm_logits, feature_norm = (
            norm_logits_with_norm[:, :-1],
            norm_logits_with_norm[:, -1] * -1,
        )
        norm_logits = tf.clip_by_value(norm_logits, -1 + self.epislon, 1 - self.epislon)
        feature_norm = tf.clip_by_value(
            feature_norm, self.min_feature_norm, self.max_feature_norm
        )
        ...
        if self.use_cosface_margin:
            ...
            arcface_logits = (
                tf.where(
                    tf.cast(y_true, dtype=tf.bool), norm_logits - margin, norm_logits
                )
                * self.scale
            )
            ...
            arcface_logits = tf.where(
                tf.cast(y_true, dtype=tf.bool),
                norm_logits * margin_cos
                - tf.sqrt(tf.maximum(1 - tf.pow(norm_logits, 2), 0.0)) * margin_sin,
                norm_logits,
            )
        ...
        arcface_loss = tf.keras.losses.categorical_crossentropy(
            y_true,
            arcface_logits,
            from_logits=self.from_logits,
            label_smoothing=self.label_smoothing,
...
class AdaFaceLoss(ArcfaceLossSimple):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.margin, self.margin_alpha, self.mean_std_alpha = (
            margin,
            margin_alpha,
            mean_std_alpha,
        )
    ...
    def __to_scaled_margin__(self, feature_norm):
        ...
        norm_std = tf.sqrt(
            tf.math.reduce_sum((feature_norm - norm_mean) ** 2) / samples
        )  # Torch std
        self.batch_mean.assign(
            self.mean_std_alpha * norm_mean
            + (1.0 - self.mean_std_alpha) * self.batch_mean
        )
        self.batch_std.assign(
            self.mean_std_alpha * norm_std
            + (1.0 - self.mean_std_alpha) * self.batch_std
        )
        margin_scaler = (feature_norm - self.batch_mean) / (
            self.batch_std + self.epislon
        )  # 66% between -1, 1
        margin_scaler = tf.clip_by_value(
            margin_scaler * self.margin_alpha, -1, 1
    ...
    def call(self, y_true, norm_logits_with_norm):
        ...
        tf.print(
            ...
            tf.histogram_fixed_width(
                scaled_margin, [-self.margin, self.margin], nbins=3
            ),
            sep="",
            end="\r",
        )
        ...
        arcface_logits = tf.where(
            ...
            tf.cos(
                tf.clip_by_value(
                    tf.acos(norm_logits) - scaled_margin,
                    self.epislon,
                    self.cos_max_epislon,
                )
            )
            - (self.margin + scaled_margin),
            norm_logits,
        )
        # arcface_logits = tf.minimum(arcface_logits, norm_logits) * self.scale
...
class AdaCosLoss(tf.keras.losses.Loss):
    def __init__(
        ...
        **kwargs
        ...
        self.max_median, self.from_logits, self.label_smoothing = (
            max_median,
            from_logits,
            label_smoothing,
        )
    ...
    def call(self, y_true, norm_logits):
        ...
        B_avg = tf.where(
            pick_cond, tf.zeros_like(norm_logits), tf.exp(self.scale * norm_logits)
        )
        B_avg = tf.reduce_mean(tf.reduce_sum(B_avg, axis=1))
        self.scale.assign(
            tf.math.log(B_avg) / tf.cos(tf.minimum(self.theta_med_max, theta_med))
        )
...
class AcrTripLoss(ArcfaceLossSimple):
    def __init__(
        ...
        super(AcrTripLoss, self).__init__(
            margin, scale, from_logits, label_smoothing, **kwargs
        )
    ...
    def call(self, y_true, outputs):
        ...
        pos_hd_margin_valid = tf.where(
            pos_hardest_dists > self.threshold,
            pos_hd_margin,
            self.theta_min - pos_hd_margin,
        )
        ...
        """ Combine """
        theta_valid = (
            tf.where(y_pred_vals > self.threshold, theta, self.theta_min - theta)
            - triplet_loss
        )
        arcface_logits = (
            tf.tensor_scatter_nd_update(norm_logits, pick_cond, theta_valid)
...
class CenterLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, embedding):
        ...
        self.centers.assign(
            tf.tensor_scatter_nd_sub(self.centers, tf.expand_dims(labels, 1), diff)
        )
        # centers_batch = tf.gather(self.centers, labels)
        return loss
...
class BatchHardTripletLossEuclidean(TripletLossWapper):
    def __calculate_triplet_loss__(self, labels, embeddings, alpha):
        ...
        dists = (
            tf.reshape(embeddings_sqaure_sum, (-1, 1)) + embeddings_sqaure_sum - 2 * ab
        )
        # pos_dists = tf.ragged.boolean_mask(dists, pos_mask)
        ...
        # neg_dists = tf.ragged.boolean_mask(dists, tf.logical_not(pos_mask))
        neg_dists = tf.where(
            pos_mask, tf.ones_like(dists) * tf.reduce_max(dists), dists
        )
...
class BatchAllTripletLoss(TripletLossWapper):
    def __calculate_triplet_loss__(self, labels, embeddings, alpha):
        ...
        pos_dists = tf.where(pos_mask, dists, tf.ones_like(dists))
        pos_dists_loss = tf.reduce_sum(1.0 - pos_dists, -1) / tf.reduce_sum(
            tf.cast(pos_mask, dtype=tf.float32), -1
        )
        pos_hardest_dists = tf.expand_dims(tf.reduce_min(pos_dists, -1), 1)

        neg_valid_mask = tf.logical_and(
            tf.logical_not(pos_mask), (pos_hardest_dists - dists) < alpha
        )
...
class OfflineTripletLoss(TripletLossWapper):
    def __calculate_triplet_loss__(self, labels, embeddings, alpha):
        ...
        anchor_emb, pos_emb, neg_emb = tf.split(
            tf.reshape(norm_emb, [-1, 3, norm_emb.shape[-1]]), 3, axis=1
        )
...
class OfflineArcTripletLoss(TripletLossWapper):
    ...
    def __calculate_triplet_loss__(self, labels, embeddings, alpha):
        ...
        pos_valid = tf.where(
            pos_margin > self.threshold, pos_margin, self.theta_min - pos_margin
        )
...
def distiller_loss_cosine(true_emb, pred_emb):
    ...
    true_emb = tf.where(
        tf.math.is_finite(true_emb), true_emb, tf.zeros_like(true_emb) + norm_one
    )
