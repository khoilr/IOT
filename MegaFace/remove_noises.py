...
def main(args):
    ...
    for line in open(args.facescrub_lst, "r"):
        ...
        feature_path = os.path.join(
            args.feature_dir_input, "facescrub", a, "%s_%s.bin" % (b, args.algo)
        )
        ...
        if not b in fs_noise_map:
            ...
            if not a in fname2center:
                fname2center[a] = np.zeros(
                    (feature_dim + feature_ext,), dtype=np.float32
                )
    ...
    for k in noises:
        ...
        feature_path_out = os.path.join(
            args.feature_dir_out, "facescrub", a, "%s_%s.bin" % (b, args.algo)
        )
    ...
    for line in open(args.megaface_lst, "r"):
        ...
        feature_path = os.path.join(
            args.feature_dir_input, "megaface", a1, a2, "%s_%s.bin" % (b, args.algo)
        )
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--facescrub-noises", type=str, help="", default="./data/facescrub_noises.txt"
    )
    parser.add_argument(
        "--megaface-noises", type=str, help="", default="./data/megaface_noises.txt"
    )
    parser.add_argument("--algo", type=str, help="", default="insightface")
    parser.add_argument(
        "--facescrub-lst", type=str, help="", default="./data/facescrub_lst"
    )
    parser.add_argument(
        "--megaface-lst", type=str, help="", default="./data/megaface_lst"
    )
    parser.add_argument(
        "--feature-dir-input", type=str, help="", default="./feature_out"
    )
    parser.add_argument(
        "--feature-dir-out", type=str, help="", default="./feature_out_clean"
    )
