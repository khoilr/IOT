...
def __init_model_from_name__(
    name, input_shape=(112, 112, 3), weights="imagenet", **kwargs
):
    ...
    if name_lower == "ghostnetv1":
        ...
        xx = ghost_model.GhostNet(
            input_shape=input_shape, include_top=False, width=1.3, **kwargs
        )

    elif name_lower == "ghostnetv2":
        from backbones import ghostv2

        xx = ghostv2.GhostNetV2(
            stem_width=16,
            # stem_strides=1,
            width_mul=1.3,
...
def buildin_models(
    ...
    **kwargs
    ...
    if (
        add_pointwise_conv
    ):  # Model using `pointwise_conv + GDC` / `pointwise_conv + E` is smaller than `E`
        filters = (
            nn.shape[-1] // 2 if add_pointwise_conv == -1 else 512
        )  # Compitable with previous models...
        nn = keras.layers.Conv2D(
            filters, 1, use_bias=False, padding="valid", name="pw_conv"
        )(nn)
        # nn = keras.layers.Conv2D(nn.shape[-1] // 2, 1, use_bias=False, padding="valid", name="pw_conv")(nn)
        nn = keras.layers.BatchNormalization(
            momentum=bn_momentum, epsilon=bn_epsilon, name="pw_bn"
        )(nn)
        if pointwise_conv_act.lower() == "prelu":
            nn = keras.layers.PReLU(
                shared_axes=[1, 2], name="pw_" + pointwise_conv_act
            )(nn)
        else:
            nn = keras.layers.Activation(
                pointwise_conv_act, name="pw_" + pointwise_conv_act
            )(nn)
    ...
    nn = keras.layers.BatchNormalization(
        momentum=bn_momentum, epsilon=bn_epsilon, name="GDC_batchnorm"
    )(nn)
    if dropout > 0 and dropout < 1:
        nn = keras.layers.Dropout(dropout)(nn)
    nn = keras.layers.Conv2D(
        emb_shape, 1, use_bias=True, kernel_initializer="glorot_normal", name="GDC_conv"
    )(nn)
    ...
    embedding = keras.layers.BatchNormalization(
        momentum=bn_momentum, epsilon=bn_epsilon, scale=scale, name="pre_embedding"
    )(nn)
...
def add_l2_regularizer_2_model(
    model,
    weight_decay,
    custom_objects={},
    ...
    if 0:
        ...
        for layer in model.layers:
            rrs = [
                kk
                for kk in layer.__dict__.keys()
                if "regularizer" in kk and not kk.startswith("_")
            ]
            if len(rrs) != 0:
    ...
    for layer in model.layers:
        ...
        if isinstance(layer, keras.layers.Dense) or isinstance(
            ...
            if apply_to_bias and layer.use_bias:
                attrs.append("bias_regularizer")
        elif apply_to_batch_normal and isinstance(
    ...
    def convert_ReLU(layer):
        ...
        if isinstance(layer, ReLU) or (
            ...
            if target_activation == "PReLU":
                ...
                print(">>>> Convert ReLU:", layer.name, "-->", layer_name)
                return Activation(
                    activation=target_activation, name=layer_name, **kwargs
                )
            else:
...
def replace_add_with_stochastic_depth(model, survivals=(1, 0.8)):
    ...
    if isinstance(survivals, float):
        ...
        start, end = survivals
        survivals = [
            start - (1 - end) * float(ii) / total_adds for ii in range(total_adds)
        ]
    ...
    return keras.models.clone_model(
        ...
        clone_function=__replace_add_with_stochastic_depth__,
    )
...
def replace_stochastic_depth_with_add(model, drop_survival=False):
        ...
        if isinstance(layer, StochasticDepth):
            ...
            if drop_survival or not survival < 1:
                ...
                return keras.layers.Lambda(
                    lambda xx: xx[0] + xx[1] * survival, name=new_layer_name
                )
        return layer
    ...
    return keras.models.clone_model(
        model,
        input_tensors=input_tensors,
        clone_function=__replace_stochastic_depth_with_add__,
    )
...
def convert_to_mixed_float16(model, convert_batch_norm=False):
    ...
    def do_convert_to_mixed_float16(layer):
        if not convert_batch_norm and isinstance(
            layer, keras.layers.BatchNormalization
        ):
            return layer
    ...
    input_tensors = keras.layers.Input(model.input_shape[1:])
    mm = keras.models.clone_model(
        model, input_tensors=input_tensors, clone_function=do_convert_to_mixed_float16
    )
    if model.built:
        mm.compile(
            optimizer=model.optimizer,
            loss=model.compiled_loss,
            metrics=model.compiled_metrics,
        )
...
def convert_mixed_float16_to_float32(model):
    ...
    def do_convert_to_mixed_float16(layer):
        if not isinstance(layer, InputLayer) and not (
            isinstance(layer, Activation) and layer.activation == linear
        ):
            aa = layer.get_config()
...
def convert_to_batch_renorm(model):
    ...
    input_tensors = keras.layers.Input(model.input_shape[1:])
    return keras.models.clone_model(
        model, input_tensors=input_tensors, clone_function=do_convert_to_batch_renorm
    )
