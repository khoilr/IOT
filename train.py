...
class Train:
    def __init__(
        ...
        self.vpl_start_iters, self.vpl_allowed_delta = (
            vpl_start_iters,
            vpl_allowed_delta,
        )
        ...
        if isinstance(model, str):
            if model.endswith(".h5") and os.path.exists(model):
                ...
                embedding_layer = (
                    basic_model if basic_model is not None else self.__search_embedding_layer__(self.model)
                )
        ...
        # Losses
        self.softmax, self.arcface, self.arcface_partial, self.triplet = (
            "softmax",
            "arcface",
            "arcface_partial",
            "triplet",
        ...
        self.data_path, self.random_status, self.image_per_class, self.mixup_alpha = (
            data_path,
            random_status,
            image_per_class,
            mixup_alpha,
        )
        self.random_cutout_mask_area, self.partial_fc_split, self.samples_per_mining = (
            random_cutout_mask_area,
            partial_fc_split,
            samples_per_mining,
        )
        self.train_ds, self.steps_per_epoch, self.classes, self.is_triplet_dataset = (
            None,
            None,
            0,
            False,
    ...
    def __init_dataset__(self, type, emb_loss_names):
        ...
        if (
            ...
            and init_as_triplet == self.is_triplet_dataset
            and not self.is_distill_ds
            and not is_offline_triplet
        ):
            return
        ...
        if is_offline_triplet:
            ...
            aa = data.Triplet_dataset_offline(
                basic_model=self.basic_model, samples_per_mining=self.samples_per_mining, **dataset_params
            )
            ...
            if self.data_path.endswith(".tfrecord"):
                ...
                ) = data.prepare_distill_dataset_tfrecord(**dataset_params)
            else:
            ...
            if self.data_path.endswith(".tfrecord"):
                ...
                self.train_ds, self.steps_per_epoch = data.prepare_dataset(
                    **dataset_params, partial_fc_split=self.partial_fc_split
                )
            self.is_triplet_dataset = False
        ...
        if isinstance(label_spec, tuple):
            ...
            elif (self.distill in emb_loss_names and len(emb_loss_names) != 1) or (
                self.distill not in emb_loss_names and len(emb_loss_names) != 0
            ):
    ...
    def __init_optimizer__(self, optimizer):
        if optimizer == None:
            if self.model != None and self.model.optimizer != None:
                ...
                print(
                    ">>>> Reuse optimizer from previoue model:",
                    compiled_opt.__class__.__name__,
                )
    ...
    def __init_model__(self, type, loss_top_k=1, header_append_norm=False):
        ...
        if type == self.softmax and model_output_layer_name != self.softmax:
        ...
        elif type == self.arcface and (
            model_output_layer_name != self.arcface or self.model.layers[-1].append_norm != header_append_norm
        ):
            vpl_start_iters = (
                self.vpl_start_iters * self.steps_per_epoch if self.vpl_start_iters < 50 else self.vpl_start_iters
            )
            vpl_kwargs = {
                "vpl_lambda": 0.15,
                "start_iters": vpl_start_iters,
                "allowed_delta": self.vpl_allowed_delta,
            }
            arc_kwargs = {
                "loss_top_k": loss_top_k,
                "append_norm": header_append_norm,
            ...
            if vpl_start_iters > 0:
                ...
                arcface_logits = models.NormDenseVPL(
                    batch_size, self.classes, output_kernel_regularizer, **arc_kwargs, **vpl_kwargs, dtype="float32"
                )
            else:
                arcface_logits = models.NormDense(
                    self.classes, output_kernel_regularizer, **arc_kwargs, dtype="float32"
                )
    ...
    def __add_emb_output_to_model__(self, emb_type, emb_loss, emb_loss_weight):
        ...
        if emb_type == self.distill and self.teacher_emb_size != emb_shape:
            print(
                ">>>> Add a dense layer to map embedding: student %d --> teacher %d"
                % (emb_shape, self.teacher_emb_size)
            )
            embedding = self.basic_model.outputs[0]
            if self.distill_emb_map_layer is None:
                self.distill_emb_map_layer = keras.layers.Dense(
        ...
        if embLossTypes is not None:
            ...
            for id, ee in enumerate(embLossTypes):
                ...
                if "centerloss" in emb_loss_name:
                    ...
                    emb_loss_names[self.distill] = (
                        losses.distiller_loss_cosine if ee == None or isinstance(ee, str) else ee
                    )
    ...
    def train_single_scheduler(
        ...
        latest_save_path = os.path.join(
            "checkpoints",
            os.path.splitext(self.save_path)[0] + "_basic_model_latest.h5",
        )
