...
class NormDense(keras.layers.Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.units, self.loss_top_k, self.append_norm, self.partial_fc_split = (
            units,
            loss_top_k,
            append_norm,
            partial_fc_split,
    ...
    def build(self, input_shape):
        if self.partial_fc_split > 1:
            ...
            self.sub_weights = self.add_weight(
                ...
                shape=(
                    self.partial_fc_split,
                    input_shape[-1],
                    self.units * self.loss_top_k,
                ),
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.append_norm:
            ...
            output = tf.concat(
                [output, tf.norm(inputs, axis=1, keepdims=True) * -1], axis=-1
            )
        return output
...
class NormDenseVPL(NormDense):
    def __init__(
        ...
        **kwargs
        ...
        self.vpl_lambda, self.batch_size = (
            vpl_lambda,
            batch_size,
        )  # Need the actual batch_size here, for storing inputs
    ...
    def build(self, input_shape):
        ...
        self.norm_features = self.add_weight(
            name="norm_features",
            shape=(self.batch_size, input_shape[-1]),
            dtype=self.compute_dtype,
            trainable=False,
        )
        self.queue_features = self.add_weight(
            name="queue_features",
            shape=(self.units, input_shape[-1]),
            initializer=self.init,
            trainable=False,
        )
        self.queue_iters = self.add_weight(
            name="queue_iters",
            shape=(self.units,),
            initializer="zeros",
            dtype="int64",
        ...
        self.iters = self.add_weight(
            name="iters", shape=(), initializer="zeros", dtype="int64", trainable=False
        )
    ...
    def call(self, inputs, **kwargs):
        ...
        queue_lambda = tf.cond(
            ...
            lambda: tf.where(
                ...
                self.vpl_lambda,
                0.0,
            ),  # prepare_queue_lambda
            lambda: self.zero_queue_lambda,
        )
        tf.print(
            " - vpl_sample_ratio:",
            tf.reduce_mean(tf.cast(queue_lambda > 0, "float32")),
            end="",
        )
        ...
        injected_weight = (
            norm_w * (1 - queue_lambda)
            + tf.transpose(self.queue_features) * queue_lambda
        )
...
class AconC(keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.p1 = self.add_weight(
            ...
            initializer=self.p1_init,
            trainable=True,
        )
        self.p2 = self.add_weight(
            name="p2",
            shape=(1, 1, 1, input_shape[-1]),
            initializer=self.p2_init,
            trainable=True,
        )
