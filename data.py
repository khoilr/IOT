...
class ImageClassesRule_map:
    def __init__(self, dir, dir_rule="*", excludes=[]):
        raw_classes = [
            os.path.basename(ii) for ii in glob2.glob(os.path.join(dir, dir_rule))
        ]
...
def pre_process_folder(data_path, image_names_reg=None, image_classes_rule=None):
    ...
    if os.path.exists(dest_pickle):
        ...
        embeddings = np.array([])
        np.savez_compressed(
            dest_pickle, image_names=image_names, image_classes=image_classes
        )
...
class RandomProcessImage:
    def __init__(
        ...
        self.img_shape, self.random_status, self.random_crop = (
            img_shape[:2],
            random_status,
            random_crop,
        )
        if random_status >= 100:
            ...
            if random_cutout_mask_area > 0:
                ...
                self.process = lambda img: aa.distort(
                    random_cutout_or_cutout_mask(
                        tf.image.random_flip_left_right(img),
                        img_shape,
                        random_cutout_mask_area,
                        random_cutout=0,
                    )
                )
            else:
            ...
            if random_cutout_mask_area > 0:
                self.process = lambda img: self.tf_buildin_image_random(
                    random_cutout_or_cutout_mask(
                        img, random_cutout_mask_area, random_cutout=0
                    )
                )
            else:
    ...
    def tf_buildin_image_random(self, img):
        ...
        if self.random_status >= 2:
            img = tf.image.random_contrast(
                img, 1 - 0.1 * self.random_status, 1 + 0.1 * self.random_status
            )
...
def random_cutout_or_cutout_mask(
    image,
    image_shape,
    random_cutout_mask_area=0.3,
    random_cutout=0,
    pad_size=20,
    replace=128,
):
    ...
    if random_cutout > 0:
        mask_func = lambda imm: tf.cond(
            ...
            lambda: tf.image.pad_to_bounding_box(
                imm[: random_height()] - replace, 0, 0, image_hh, image_ww
            )
            + replace,
...
def mixup(image, label, alpha=0.4):
    ...
    image = image * img_mix_weight + tf.gather(image, shuffle_index) * (
        1.0 - img_mix_weight
    )
    label = tf.cast(label, "float32")
    label = label * label_mix_weight + tf.gather(label, shuffle_index) * (
        1 - label_mix_weight
    )
    return image, label
...
class MXNetRecordGen:
    def __init__(self, data_path):
        ...
        self.imgrec, self.rec_header, self.classes, self.total_images = (
            imgrec,
            rec_header,
            classes,
            total_images,
...
def show_batch_sample(ds, rows=8, basic_size=1):
    ...
    plt.imshow(
        np.vstack(
            [np.hstack(aa[ii * columns : (ii + 1) * columns]) for ii in range(rows)]
        )
    )
    plt.axis("off")
    plt.tight_layout()
...
def partial_fc_split_pick(image_names, image_classes, batch_size, split=2, debug=False):
    ...
    splits = np.array(
        [classes // split * ii for ii in range(split + 1)]
    )  # Drop class if cannot divided, keep output shape concurrent

    shuffle_indexes = np.random.permutation(total)
    image_names, image_classes = (
        image_names[shuffle_indexes],
        image_classes[shuffle_indexes],
    )

    picks = [
        np.logical_and(image_classes >= splits[ii], image_classes < splits[ii + 1])
    ...
    if debug:
        print(
            ...
            [ii.sum() for ii in picks],
        )

    indexes = np.arange(len(image_classes))
    split_index = [
        indexes[ii][: ii.sum() // batch_size * batch_size].reshape(-1, batch_size)
        for ii in picks
    ]
    if debug:
        print(
            ">>>> After drop remainder:",
            [ii.shape for ii in split_index],
            ", prod:",
            [np.prod(ii.shape) for ii in split_index],
        )
    ...
    split_index = np.concatenate(
        [np.expand_dims(ii[:min_batches], 1) for ii in split_index], axis=1
    )
    split_index = split_index.ravel()  # flatten
    ...
    if debug:
        ...
        for ii in range(bb.shape[0] // batch_size):
            ...
            rrs.append(
                np.alltrue(
                    np.logical_and(
                        batch >= splits[split_id], batch < splits[split_id + 1]
                    )
                )
            )
        print(
            ">>>> Total batches:",
            bb.shape[0] // batch_size,
            ", correctly split:",
            np.sum(rrs),
        )
...
def partial_fc_split_gen(image_names, image_classes, batch_size, split=2, debug=False):
    while True:
        for image_name, image_class in zip(
            *partial_fc_split_pick(image_names, image_classes, batch_size, split, debug)
        ):
...
def prepare_dataset(
    ...
    image_names, image_classes, embeddings, classes, _ = pre_process_folder(
        data_path, image_names_reg, image_classes_rule
    )
    total_images = len(image_names)
    if total_images == 0:
        print(
            ">>>> [Error] total_images is 0, image_names:",
            image_names,
            "image_classes:",
            image_classes,
        )
        return None, None
    print(
        ">>>> Image length: %d, Image class length: %d, classes: %d"
        % (len(image_names), len(image_classes), classes)
    )
    if image_per_class != 0:
        ...
        print(
            ">>>> After pick[%d], images: %d, valid classes: %d"
            % (image_per_class, len(image_names), class_pick.shape[0])
        )
    ...
    if len(embeddings) != 0 and teacher_model_interf is None:
        ...
        ds = tf.data.Dataset.from_tensor_slices(
            (image_names, embeddings, image_classes)
        ).shuffle(buffer_size=total_images)
        process_func = lambda imm, emb, label: (
            tf_imread(imm),
            (emb, tf.one_hot(label, depth=classes, dtype=tf.int32)),
        )
    elif partial_fc_split != 0:
        print(">>>> partial_fc_split provided:", partial_fc_split)
        picked_images, _ = partial_fc_split_pick(
            image_names, image_classes, batch_size, split=partial_fc_split, debug=True
        )
        total_images = picked_images.shape[0]
        sub_classes = classes // partial_fc_split
        print(
            ">>>> total images after pick: {}, sub_classes: {}".format(
                total_images, sub_classes
            )
        )

        gen_func = lambda: partial_fc_split_gen(
            image_names, image_classes, batch_size, split=partial_fc_split
        )
        output_signature = (
        ...
        process_func = lambda imm, label: (
            tf_imread(imm),
            tf.one_hot(label % sub_classes, depth=sub_classes, dtype=tf.int32),
        )
        # process_func = lambda imm, label: (tf_imread(imm), tf.one_hot(label, depth=classes, dtype=tf.int32))
    else:
        ds = tf.data.Dataset.from_tensor_slices((image_names, image_classes)).shuffle(
            buffer_size=total_images
        )
        process_func = lambda imm, label: (
            tf_imread(imm),
            tf.one_hot(label, depth=classes, dtype=tf.int32),
        )
    ...
    if is_train and random_status >= 0:
        random_process_image = RandomProcessImage(
            img_shape, random_status, random_crop, random_cutout_mask_area
        )
    ...
    )  # Use batch --> map has slightly effect on dataset reading time, but harm the randomness
    if mixup_alpha > 0 and mixup_alpha <= 1:
        print(">>>> mixup_alpha provided:", mixup_alpha)
        ds = ds.map(
            lambda xx, yy: mixup((xx - 127.5) * 0.0078125, yy, alpha=mixup_alpha)
        )
    else:
...
def prepare_distill_dataset_tfrecord(
    ...
    **kw,
    ...
    def decode_fn(record_bytes):
        ...
        image_name, image_classe, embedding = (
            ff["image_names"],
            ff["image_classes"],
            ff["embeddings"],
        )
...
class Triplet_dataset:
    def __init__(
        ...
        **kw,
        ...
        AUTOTUNE = tf.data.experimental.AUTOTUNE
        self.image_classes_rule = (
            ImageClassesRule_map(data_path)
            if image_classes_rule is None
            else image_classes_rule
        )
        image_names, image_classes, embeddings, classes, _ = pre_process_folder(
            data_path, image_names_reg, self.image_classes_rule
        )
        ...
        image_dataframe = pd.DataFrame(
            {"image_names": image_names, "image_classes": image_classes}
        )
        self.image_dataframe = image_dataframe.groupby("image_classes").apply(
            lambda xx: xx.image_names.values
        )
        self.split_func = lambda xx: np.array(
            np.split(
                np.random.permutation(xx)[
                    : len(xx) // image_per_class * image_per_class
                ],
                len(xx) // image_per_class,
            )
        )
        ...
        print(
            "The final train_dataset batch will be %s"
            % ([self.batch_size, *self.img_shape, self.channels])
        )
        ...
        if len(embeddings) != 0 and teacher_model_interf is None:
            ...
            ds = tf.data.Dataset.from_generator(
                self.image_shuffle_gen_with_emb, output_signature=output_signature
            )
            process_func = lambda imm, emb, label: (
                random_imread(imm),
                (emb, one_hot_label(label)),
            )
        else:
            output_signature = (
                tf.TensorSpec(shape=(), dtype=tf.string),
            ...
            ds = tf.data.Dataset.from_generator(
                self.image_shuffle_gen, output_signature=output_signature
            )
    ...
    def image_shuffle_gen(self):
        while True:
            ...
            shuffle_dataset = self.image_dataframe.map(self.split_func)
            image_data = np.random.permutation(
                np.vstack(shuffle_dataset.values)
            ).flatten()
...
class Triplet_dataset_offline:
    def __init__(
        ...
        **kwargs,
        ...
        pick, _ = pick_by_image_per_class(image_classes, image_per_class)
        image_names, image_classes = image_names[pick].astype(str), image_classes[pick]
        self.image_names, self.image_classes, self.classes = (
            image_names,
            image_classes,
            classes,
        )
        ...
        if samples_per_mining > 1:
            ...
            self.samples_per_mining = (
                int(image_names.shape[0] * samples_per_mining) // image_per_class
            )
        else:
        ...
        ds = tf.data.Dataset.from_generator(
            self.offline_triplet_mining, output_signature=output_signature
        ).repeat()
        process_func = lambda imm, label: (random_imread(imm), one_hot_label(label))
        ds = ds.map(process_func, num_parallel_calls=AUTOTUNE)
        ...
        total = (
            self.samples_per_mining * image_per_class
            if self.samples_per_mining > 0
            else len(image_classes)
        )
        print(">>>> total:", total)
    ...
    def offline_triplet_mining(self):
        if self.samples_per_mining > 0:
            shuffle_dataset = self.image_dataframe.map(self.split_func)
            image_names = np.random.permutation(np.vstack(shuffle_dataset.values))[
                : self.samples_per_mining
            ].flatten()
        else:
        ...
        ds = (
            ds.map(lambda xx: (tf_imread(xx) - 127.5) * 0.0078125)
        ...
        labels = tf.convert_to_tensor(
            [self.image_classes_rule(ii) for ii in image_names]
        )
        ...
        for batch_id in range(total_batch):
            ...
            pos_mask = tf.equal(
                tf.expand_dims(labels, 1), tf.expand_dims(cur_labels, 0)
            )
            ...
            for anchor_image, anchor_label, pos, neg in zip(
                cur_image_names, cur_labels, hardest_pos_idxes, hardest_neg_idxes
            ):
