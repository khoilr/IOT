...
class EvalCallback(tf.keras.callbacks.Callback):
    def __init__(
        ...
        self.save_model, self.eval_freq, self.flip, self.PCA_acc = (
            save_model,
            eval_freq,
            flip,
            PCA_acc,
    ...
    def __eval_func__(self, cur_step=0, logs=None, eval_freq=1):
        ...
        if acc_max >= self.max_accuracy:
            ...
            if self.save_model:
                save_name_base = "%s_basic_%s_epoch_" % (
                    self.save_model,
                    self.test_names,
                )
...
if __name__ == "__main__":
    ...
    for test_bin_file in args.test_bin_files:
        aa = EvalCallback(basic_model, test_bin_file, batch_size=args.batch_size, flip=flip)
        aa.on_epoch_end()
