...
class VPLUpdateQueue(keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        updated_queue = tf.tensor_scatter_nd_update(
            vpl_norm_dense_layer.queue_features,
            update_label_pos,
            vpl_norm_dense_layer.norm_features,
        )
...
class CosineLrSchedulerEpoch(keras.callbacks.Callback):
    def __init__(
        ...
        self.warmup_steps, self.cooldown_steps, self.lr_min = (
            warmup_steps,
            cooldown_steps,
            lr_min,
        )
        ...
        if lr_min == lr_base * m_mul:
            ...
            self.schedule = keras.experimental.CosineDecayRestarts(
                ...
                t_mul=t_mul,
                m_mul=m_mul,
                alpha=lr_min / lr_base,
            )
            aa = [first_restart_step * (t_mul**ii) for ii in range(5)]
            self.cooldown_steps_start = np.array(
                [warmup_steps + int(sum(aa[:ii]) + cooldown_steps * (ii - 1)) for ii in range(1, 5)]
            )
...
class CosineLrScheduler(keras.callbacks.Callback):
    def __init__(
        ...
        self.lr_base, self.m_mul, self.t_mul, self.lr_min, self.steps_per_epoch = (
            lr_base,
            m_mul,
            t_mul,
            lr_min,
        ...
        (
            ...
            self.cooldown_steps,
            self.lr_warmup,
        ) = (first_restart_step, warmup_steps, cooldown_steps, lr_warmup)
        (
            self.init_step_num,
            self.cur_epoch,
            self.is_cooldown_epoch,
            self.previous_cooldown_steps,
        ) = (0, 0, False, 0)
        self.is_built = False
    ...
    def build(self, steps_per_epoch=-1):
        ...
        if self.lr_min == self.lr_base * self.m_mul:  # Without restart
            ...
            self.cooldown_steps_start = np.array(
                [self.warmup_steps + int(sum(aa[:ii]) + self.cooldown_steps * (ii - 1)) for ii in range(1, 5)]
            )
        ...
        if self.warmup_steps != 0:
            ...
            self.warmup_lr_func = (
                lambda ii: self.lr_warmup + (self.lr_base - self.lr_warmup) * ii / self.warmup_batch_steps
            )
        else:
