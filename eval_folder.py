...
class Eval_folder:
    def __init__(self, model_interf, data_path, batch_size=128, save_embeddings=None):
        ...
        (
            ...
            self.imm_classes,
            self.filenames,
        ) = self.prepare_images_and_embeddings(data_path, batch_size, save_embeddings)
    ...
    def generate_eval_pair_bin(self, save_dest, pos_num=3000, neg_num=3000, min_pos=0, max_neg=1.0, nfold=10):
        ...
        p1_images, p2_images = (
            np.array(p1_images)[pos_score_cond],
            np.array(p2_images)[pos_score_cond],
        )
        n1_images, n2_images = (
            np.array(n1_images)[neg_score_cond],
            np.array(n2_images)[neg_score_cond],
        )

        """ pick by sorted score values """
        pos_pick_cond = np.argsort(pos_scores)[:pos_num]
        neg_pick_cond = np.argsort(neg_scores)[-neg_num:]
        pos_scores, p1_images, p2_images = (
            pos_scores[pos_pick_cond],
            p1_images[pos_pick_cond],
            p2_images[pos_pick_cond],
        )
        neg_scores, n1_images, n2_images = (
            neg_scores[neg_pick_cond],
            n1_images[neg_pick_cond],
            n2_images[neg_pick_cond],
        )

        bins = []
        total = pos_num + neg_num
        for img_1, img_2 in tqdm(
            list(zip(p1_images, p2_images)) + list(zip(n1_images, n2_images)),
            "Creating bins",
            total=total,
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Data path, containing images in class folders",
    )
    parser.add_argument(
        "-m",
        "--model_file",
    ...
    parser.add_argument(
        ...
        "--detection",
        action="store_true",
        help="Run face detection before embedding",
    )
    parser.add_argument(
        "-S",
        "--save_embeddings",
        type=str,
        default=None,
        help="Save / Reload embeddings data",
    )
    parser.add_argument(
        "-B",
        "--save_bins",
