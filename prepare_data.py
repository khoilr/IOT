...
def resize_dataset(dataset_dir, target_shape=224):
    ...
    for ii in tqdm(aa):
        ...
        cv2.imwrite(
            target_imm,
            cv2.resize(imm, (target_shape, target_shape), interpolation=cv2.INTER_CUBIC),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="MXnet record dataset directory",
    )
    parser.add_argument(
        "-T",
        "--test_bins",
        nargs="*",
        type=None,
        help="Test bin files in dataset_dir be converted",
    )
    parser.add_argument(
        "-S",
        "--save_dir",
        default=None,
        help="Folder path for saving dataset images",
    )
