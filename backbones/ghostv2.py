...
def ghost_module(inputs, out_channel, activation="relu", name=""):
    ...
    primary_conv = batchnorm_with_activation(
        primary_conv, activation=activation, name=name + "prim_"
    )
    ...
    cheap_conv = depthwise_conv2d_no_bias(
        primary_conv, kernel_size=3, padding="SAME", name=name + "cheap_"
    )
    cheap_conv = batchnorm_with_activation(
        cheap_conv, activation=activation, name=name + "cheap_"
    )
...
def ghost_module_multiply(inputs, out_channel, activation="relu", name=""):
    ...
    shortcut = batchnorm_with_activation(
        shortcut, activation=None, name=name + "short_1_"
    )
    shortcut = depthwise_conv2d_no_bias(
        shortcut, (1, 5), padding="SAME", name=name + "short_2_"
    )
    shortcut = batchnorm_with_activation(
        shortcut, activation=None, name=name + "short_2_"
    )
    shortcut = depthwise_conv2d_no_bias(
        shortcut, (5, 1), padding="SAME", name=name + "short_3_"
    )
    shortcut = batchnorm_with_activation(
        shortcut, activation=None, name=name + "short_3_"
    )
    shortcut = activation_by_name(shortcut, "sigmoid", name=name + "short_")
    # print(f"{shortcut.shape = }, {nn.shape = }")
    shortcut = tf.image.resize(
        shortcut, tf.shape(inputs)[1:-1], antialias=False, method="bilinear"
    )
...
def ghost_bottleneck(
    ...
    if shortcut:
        shortcut = depthwise_conv2d_no_bias(
            inputs, kernel_size, strides, padding="same", name=name + "short_1_"
        )
        shortcut = batchnorm_with_activation(
            shortcut, activation=None, name=name + "short_1_"
        )
    ...
    if use_ghost_module_multiply:
        nn = ghost_module_multiply(
            inputs, first_ghost_channel, activation=activation, name=name + "ghost_1_"
        )
    else:
        nn = ghost_module(
            inputs, first_ghost_channel, activation=activation, name=name + "ghost_1_"
    ...
    if strides > 1:
        nn = depthwise_conv2d_no_bias(
            nn, kernel_size, strides, padding="same", name=name + "down_"
        )
    ...
    if se_ratio > 0:
        nn = se_module(
            ...
            se_ratio=se_ratio,
            divisor=4,
            activation=("relu", "hard_sigmoid_torch"),
            name=name + "se_",
        )
...
def GhostNetV2(
    ...
    nn = conv2d_no_bias(
        inputs, stem_width, 3, strides=stem_strides, padding="same", name="stem_"
    )
    nn = batchnorm_with_activation(nn, activation=activation, name="stem_")

    """ stages """
    kernel_sizes = [3, 3, 3, 5, 5, 3, 3, 3, 3, 3, 3, 5, 5, 5, 5, 5]
    first_ghost_channels = [
        16,
        48,
        72,
        72,
    ...
    for stack_id, (kernel, stride, first_ghost, out_channel, se_ratio) in enumerate(
        zip(kernel_sizes, strides, first_ghost_channels, out_channels, se_ratios)
    ):
        ...
        use_ghost_module_multiply = (
            True
            if num_ghost_module_v1_stacks >= 0
    ...
    nn = conv2d_no_bias(
        nn,
        make_divisible(first_ghost_channels[-1] * width_mul, 4),
        1,
        strides=1,
        name="pre_",
    )
    ...
    if num_classes > 0:
        ...
        nn = keras.layers.Dense(
            num_classes, dtype="float32", activation=classifier_activation, name="head"
        )(nn)
