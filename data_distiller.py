...
class Data_distiller:
    def __init__(
        ...
        self.data_path, self.model_file, self.batch_size = (
            data_path,
            model_file,
            batch_size,
        )
        self.dest_file, self.save_npz, self.use_fp16, self.limit = (
            dest_file,
            save_npz,
            use_fp16,
            limit,
    ...
    def __init_ds_model_dest__(self):
        ...
        (
            ...
            _,
            classes,
            dataset_pickle_file_src,
        ) = pre_process_folder(self.data_path)
        print(
        ...
        if self.limit > 0:
            image_names, image_classes = (
                image_names[: self.limit],
                image_classes[: self.limit],
            )
        ...
        self.dest_file = (
            self.dest_file
            if self.dest_file.endswith(ext_format)
            else self.dest_file + ext_format
        )
        self.interf_func, self.ds, self.classes, self.emb_shape, self.total = (
            interf_func,
            ds,
            classes,
            emb_shape,
    ...
    def __init_from_saved_npz__(self):
        ...
        embeddings = (
            ...
            if self.use_fp16
            else embeddings.astype("float32")
        )
        print(
            ">>>> [Base info] total:",
            total,
        ...
        self.dest_file = (
            self.dest_file
            if self.dest_file.endswith(".tfrecord")
            else self.dest_file + ".tfrecord"
        )
    ...
    def __extract_emb_gen__(self):
        for imm, label in self.ds:
            ...
            emb = (
                np.array(emb, dtype="float16")
                if self.use_fp16
                else np.array(emb, dtype="float32")
            )
    ...
    def __save_to_tfrecord_by_batch__(self):
        ...
        self.encode_base_info = {
            "classes": tf.train.Feature(
                int64_list=tf.train.Int64List(value=[self.classes])
            ),
            "emb_shape": tf.train.Feature(
                int64_list=tf.train.Int64List(value=[self.emb_shape])
            ),
            "total": tf.train.Feature(
                int64_list=tf.train.Int64List(value=[self.total])
            ),
            "use_fp16": tf.train.Feature(
                int64_list=tf.train.Int64List(value=[self.use_fp16])
            ),
        }
        self.encode_feature = {
            "image_names": lambda vv: tf.train.Feature(
                bytes_list=tf.train.BytesList(value=[vv])
            ),
            "image_classes": lambda vv: tf.train.Feature(
                int64_list=tf.train.Int64List(value=[vv])
            ),
            # "embeddings": lambda vv: tf.train.Feature(float_list=tf.train.FloatList(value=vv.tolist())),
            "embeddings": lambda vv: tf.train.Feature(
                bytes_list=tf.train.BytesList(value=[vv.tobytes()])
            ),
        }

        is_first_line = True
        with tf.io.TFRecordWriter(self.dest_file) as file_writer, tqdm(
            desc=self.tqdm_desc, total=self.total
        ) as pbar:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Data path, or npz file converting to tfrecord",
    )
    parser.add_argument(
        "-M",
        "--model_file",
        type=str,
        default=None,
        help="Model file, keras h5 / pytorch pth / mxnet",
    )
    parser.add_argument(
        "-d",
        "--dest_file",
        type=str,
        default=None,
        help="Dest file path to save the processed dataset",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Test parameter, limit converting only the first [NUM]",
    )
    parser.add_argument("--use_fp16", action="store_true", help="Save using float16")
    parser.add_argument(
        "--save_npz", action="store_true", help="Save as npz file, default is tfrecord"
    )
